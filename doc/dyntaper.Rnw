...
@
\noindent
Although less intuitive, I might rather use a log transform. It may be more homoscedastic, and produces the same value for converting from outside to inside bark or vice-versa:
<<>>=
